...
import { updateProperty } from "@/app/profile/actions";
...
export default function EditProperty() {
  ...
  useEffect(() => {
    const fetchProperty = async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:5000/api/v1'}/property/${propertyId}`);
        ...
        setSubmitStatus("Failed to load property data");
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!Token?.accessToken && !Token?.refreshToken) {
        setSubmitStatus("Please log in to edit a property.");
        router.push("/login");
        return;
      }
      ...
      setSubmitStatus("Property updated successfully!");
      setTimeout(() => {
        router.push("/profile");
      }, 1500);
    } catch (error) {
      console.error("Error updating property:", error);
      setSubmitStatus("Failed to update property. Please try again.");
    } finally {
          ...
          >
            ...
            <div className="pt-4 flex gap-4">
              <button
                type="button"
                onClick={() => router.back()}
                className="flex-1 py-4 rounded-lg font-semibold transition-all duration-300 cursor-pointer whitespace-nowrap bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600"
              >
                Cancel
              </button>
              <button
                ...
                className={`flex-1 py-4 rounded-lg font-semibold transition-all duration-300 cursor-pointer whitespace-nowrap ${
                  isSubmitting || uploadingImages.some((uploading) => uploading)
